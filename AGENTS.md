...
Stdio MCP server in Go that exposes YNAB budget operations as tools. Used with Claude Desktop, Claude Code, and other MCP clients.
...
- `config.go` — Reads `~/.config/ynab-mcp/config` (KEY=VALUE), env vars take precedence
